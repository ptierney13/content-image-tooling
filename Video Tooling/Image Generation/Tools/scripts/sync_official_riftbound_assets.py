...
DEFAULT_OUTPUT_DIR = IMAGE_GENERATION_DIR / "Data" / "card-cache" / "riftbound-official"
...
def _selected_card_names(decklist_files: list[Path], explicit_cards: list[str]) -> list[str]:
...
def main() -> int:
    ...
    parser.add_argument("--decklist-file", action="append", default=[], help="Optional plaintext decklist file. Can be provided multiple times.")
    ...
    decklist_files = [Path(decklist_file).resolve() for decklist_file in args.decklist_file]
    selected_names = _selected_card_names(decklist_files, list(args.card))
