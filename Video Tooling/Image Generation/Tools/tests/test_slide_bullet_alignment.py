...
class BulletAlignmentTests(unittest.TestCase):
    ...
    def test_informative_slide_can_render_section_columns_below_top_bullets(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            deck_dir = Path(temp_dir)
            output_path = deck_dir / "sections.png"
            plan = {
                "type": "informative",
                "title": "Verify Matchup Guide",
                "output_path": str(output_path),
                "background": {"kind": "pattern"},
                "body_font_size": 36,
                "text_top": 200,
                "top_bullets_bottom": 340,
                "sections_top": 430,
                "sections_bottom": 900,
                "bullets": [
                    "Win Game 1",
            ...
            recorded_icons: list[tuple[int, int]] = []

            original_draw_bullet_icon = rip._draw_bullet_icon

            def capture_bullet_icon(draw: ImageDraw.ImageDraw, center: tuple[int, int], style: str) -> None:
                recorded_icons.append(center)
                original_draw_bullet_icon(draw, center, style)

            with patch.object(rip, "_draw_bullet_icon", side_effect=capture_bullet_icon):
                rip._draw_informative_slide(deck_dir, {}, output_path, "Verify Matchup Guide", plan, resolver)

            self.assertTrue(output_path.exists())
            self.assertEqual(len(recorded_icons), 6)
            self.assertLess(recorded_icons[0][1], recorded_icons[-1][1])
